...
def parser(url: str, low_price: int = 1, top_price: int = 1000000, discount: int = 0, min_rating: float = 0):
    """Основная функция с фильтрацией по минимальному рейтингу"""
    ...
    try:
        ...
        for page in range(1, 51):  # вб отдает 50 страниц товара (раньше было 100)
            ...
            # Передаем минимальный рейтинг для фильтрации товаров
            extracted_data = get_data_from_json(data, min_rating=min_rating)
        ...
        save_excel(data_list, f'{category["name"]}_from_{low_price}_to_{top_price}_rating_{min_rating}')
        print(
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            parser(url=url, low_price=low_price, top_price=top_price, discount=discount, min_rating=min_rating)
        except ValueError:
